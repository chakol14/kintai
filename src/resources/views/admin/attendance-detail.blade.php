...
<section class="attendance-detail">
    ...
    @if(session('status'))
        <p class="detail-warning">{{ session('status') }}</p>
    @endif
    ...
    <form class="detail-form" action="{{ route('admin.attendance.update', ['user' => $user->id, 'date' => $workDate->format('Y-m-d')]) }}" method="post">
        @csrf
        <div class="detail-table">
            ...
            <div class="detail-row">
                ...
                <div class="detail-value detail-range">
                    <input class="detail-input" type="time" name="break_start" value="{{ old('break_start', $breaks[0]['start'] ?? '') }}" {{ $isPending ? 'disabled' : '' }}>
                    <span class="detail-separator">〜</span>
                    <input class="detail-input" type="time" name="break_end" value="{{ old('break_end', $breaks[0]['end'] ?? '') }}" {{ $isPending ? 'disabled' : '' }}>
                </div>
            </div>
            <div class="detail-row">
            ...
            <div class="detail-row">
                ...
                <div class="detail-value">
                    <textarea class="detail-textarea" name="remark" rows="2" {{ $isPending ? 'disabled' : '' }}>{{ old('remark', $remark) }}</textarea>
                </div>
            </div>
        </div>
